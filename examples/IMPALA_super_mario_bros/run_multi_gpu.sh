...
SCRIPT_DIR='/opt/tiger/test_ppo/examples/IMPALA_super_mario_bros'
...
if [[ $role != "server" ]]; then
  ...
  ray start --head --include-webui --node-ip-address=$SERVER_IP --redis-port=${REDIS_PORT} --num-cpus=$((ARNOLD_SERVER_CPU - cpu_remain_for_head)) --memory=$(($ARNOLD_SERVER_MEM * 1024 ** 2)) --object-store-memory=$((ARNOLD_SERVER_GPU * 16 * 1024 ** 3)) --plasma-directory $PlASMA_DIR
  ...
  sleep 10s
  ### sleep forever, need to replace the true command here
  #cd ${GAMEBOT_DIR} && ./train.py --ray-address ${SERVER_IP}:${REDIS_PORT} --run PPO --env SuperMarioEnv --checkpoint-freq 10 --config '{"num_workers": 10}'
  # python3 $SCRIPT_DIR/test_ray_helper.py &
  nof_evaluator_on_worker=$((ARNOLD_WORKER_NUM * ARNOLD_WORKER_CPU / 4))
  nof_evaluator_on_server=$(((ARNOLD_SERVER_CPU - 4 - 1) / 4))
  ...
  bash $SCRIPT_DIR/MPIRUN.sh python3 $SCRIPT_DIR/ray_trainer_hvd.py "$@" --nof_evaluator $nof_evaluator
  echo 'bash '$SCRIPT_DIR'/MPIRUN.sh python3 '$SCRIPT_DIR'/ray_trainer_hvd.py '"$@" > /opt/tiger/test_ppo/cache_run_script.sh
  while true; do
    sleep 86400
  done
fi

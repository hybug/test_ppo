...
flags.DEFINE_string("dir", "r2d3_2", "dir number")
flags.DEFINE_string(
    "datadir", "/mnt/mytmpfs", "data dir")
flags.DEFINE_string(
    "scriptdir", "/opt/tiger/test_ppo/zR2D3_boots", "script dir")
...
def make_demo(sess, dphs, enqueue_op):
    ...
    for name in names:
        ...
        dicseg["r"] = get_n_step_rewards(dicseg["r"], n_step, gamma)
        while len(dicseg["s"]) > burn_in:
            ...
            next_seg["r"] = padding(dicseg["r"][:seqlen], seqlen, dicseg["r"][0], np.float32)
            next_seg["state_in"] = np.zeros((256 * 2), np.float32)
            next_seg["slots"] = padding([1] * len(dicseg["s"][:-1][:seqlen]), seqlen, 0, np.int32)
            next_seg["bootstrap_s"] = padding(
                dicseg["s"][seqlen:seqlen + n_step], n_step, dicseg["s"][0], np.float32)
            next_seg["bootstrap_prev_a"] = padding(
                dicseg["a"][seqlen:seqlen + n_step], n_step, dicseg["a"][0], np.int32)
...
def train():
    ...
    for i in range(FLAGS.num_servers):
        os.system("python3 %s/Server_r2d3.py "
                  "-server_id %d "
                  "-num_servers %d "
                  "-SCRIPT_DIR %s "
                  "-BASE_DIR %s "
    ...
    rep_prephs, rep_postphs = get_phs(dequeued)
    ...
    prephs = {k: tf.concat([v, demo_prephs[k]], axis=0) for k, v in rep_prephs.items()}
    postphs = {k: tf.concat([v, demo_postphs[k]], axis=0) for k, v in rep_postphs.items()}

...
def _warp_env():
    ...
    class Env(object):
        ...
        def get_history(self):
            if self.done:
                seg = Seg(self.s, self.a, self.a_logits, self.r, self.v_cur, self.state_in, self.mask)
                return self.postprocess(seg)
            elif len(self.s) > self.burn_in + self.seqlen + 1:
                cut = self.burn_in + self.seqlen + 1
                ...
                self.a_logits = self.a_logits[self.burn_in:]
                self.r = self.r[self.burn_in:]
                self.v_cur = self.v_cur[self.burn_in:]
        ...
        def postprocess_one_seg(self, seg):
            seqlen = self.seqlen + self.burn_in + 1
    ...
    class Envs(object):
        ...
        def step(self, sess, model):
            ...
            for i, env in enumerate(self.envs):
                segs += env.step(a[i][0], a_logits[i][0],

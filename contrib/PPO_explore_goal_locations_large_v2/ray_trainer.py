...
flags.DEFINE_integer("act_space", 6, "act space")

flags.DEFINE_string(
    "basedir_ceph", "/mnt/cephfs_new_wj/arnold/labcv/xiaochangnan"
                    "/dmlab", "base dir for ceph")
flags.DEFINE_string(
    "basedir_hdfs",
    'hdfs://haruna/home/byte_arnold_lq_mlsys/user/xiaochangnan/dmlab',
    "base dir for hdfs")
flags.DEFINE_string("dir", "0", "dir number")
flags.DEFINE_string(
    "scriptdir",
    "/opt/tiger/test_ppo/contrib/PPO_explore_goal_locations_large_v2",
    "script dir")

flags.DEFINE_bool("use_stage", True, "whether to use tf.contrib.staging")

flags.DEFINE_bool("use_rmc", False, "whether to use rmcrnn instead of lstm")
flags.DEFINE_bool("use_hrmc", True, "whether to use tmp hierarchy rmcrnn")
# flags.DEFINE_bool("use_rssm", False, "whether to use recurrent sequential state model")
...
flags.DEFINE_integer("parallel", 4, "parallel envs")
...
from contrib.PPO_explore_goal_locations_large_v2.policy_graph import warp_Model
...
def build_policy_evaluator(kwargs):
    ...
    if kwargs["use_hrmc"]:
        kwargs["state_size"] = 1 + (8 + 4 + 8) * 4 * 64
    elif kwargs["use_rmc"]:
    ...
    env_kwargs['action_repeats'] = [1, 2]

    model_kwargs = deepcopy(kwargs)
    # pickle func in func
    from contrib.PPO_explore_goal_locations_large_v2.env import build_env
    from contrib.PPO_explore_goal_locations_large_v2.policy_graph import build_evaluator_model

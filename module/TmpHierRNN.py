...
class TmpHierRNNCell(DropoutRNNCellMixin, Layer):
    def __init__(self,
                 time_scale,
                 size_per_head,
                 num_attention_heads,
                 ...
                 recurrent_dropout=0.,
                 **kwargs):
        super(TmpHierRNNCell, self).__init__(**kwargs)
        self.time_scale = time_scale
        self.units = num_attention_heads * size_per_head
        self.prior_memory_slots = prior_memory_slots
        self.posterior_memory_slots = posterior_memory_slots
        self.prior_core = prior_core
        if prior_core == 'lstm':
            self.prior_memory_slots = 2
        if posterior_core == 'lstm':
            self.posterior_memory_slots = 2
        self.posterior_core = posterior_core
        ...
        # properly.
        self.state_size = [
            1, (8 + self.prior_memory_slots + self.posterior_memory_slots) * self.units]
    ...
    def build(self, input_shape):
        ...
        with tf.variable_scope("prior"):
            self.p_ws = self._build_weights(
                input_dim=2 * self.units,
                core=self.prior_core,
                memory_slots=self.prior_memory_slots)
        with tf.variable_scope("posterior"):
            self.q_ws = self._build_weights(
                input_dim=input_dim + 3 * self.units,
                core=self.posterior_core,
                memory_slots=self.posterior_memory_slots)
        self.built = True

    def _build_weights(self, input_dim, core='rmc', memory_slots=None):
        d = collections.OrderedDict()
        if core == 'rmc':
            assert memory_slots is not None
            if self.use_relative_position:
                d['rel_table'] = self._make_relative_position_embedding_table(
                    memory_slots)
                d['pos_table'] = self._make_relative_position_table(
                    memory_slots)
            ...
            raise ValueError("Cannot Support %s core" % core)
        return d

    def _make_relative_position_embedding_table(self, num_memory_slots):
        table = []
        for p in range(-num_memory_slots, num_memory_slots + 1):
            row = []
    ...
    def _make_relative_position_table(self, num_memory_slots):
        from_position = np.arange(num_memory_slots + 1)[:, None]
        to_position = np.arange(num_memory_slots + 1)[None, :]
        table = from_position - to_position + num_memory_slots
        return K.cast(K.one_hot(table, 2 * num_memory_slots + 1), tf.float32)

    def _attention_layer(self, memory_plus_inputs, ws, num_memory_slots):
        from_length = num_memory_slots + 1
        to_length = num_memory_slots + 1
        ...
        if self.use_relative_position:
            # [F+T-1, N*H]
            r = K.dot(ws["rel_table"], ws["rel_kernel"])
            # [F+T-1, N, H]
            r = array_ops.reshape(
            ...
            # [B*N, F, T]
            bd = tf.einsum("bfl,ftl->bft", bd, ws["pos_table"])
            # [B*N, F, T]
            attention_scores += bd
    ...
    def _attend_over_memory(self, inputs, memory, ws, num_memory_slots):
        ...
        context_layer = self._attention_layer(
            memory_plus_inputs, ws, num_memory_slots)
        ...
        new_memory, outputs = array_ops.split(
            context_layer, [num_memory_slots, 1], axis=1)
    ...
    def _call_rmc_core(self, inputs, flatten_memory, training, ws, num_memory_slots):
        ...
        memory = array_ops.reshape(
            flatten_memory, shape=[-1, num_memory_slots, self.units])

        input_gate, forget_gate = self._input_and_forget_gates(inputs, memory, ws)
        hs, new_memory = self._attend_over_memory(
            inputs, memory, ws, num_memory_slots)

        next_memory = input_gate * new_memory + forget_gate * memory

        flatten_next_memory = array_ops.reshape(
            next_memory, shape=[-1, num_memory_slots * self.units])
    ...
    def get_config(self):
        ...
        base_config = super(TmpHierRNNCell, self).get_config()
...
class TmpHierRNN(RNN):
    def __init__(self,
        ...
        cell = TmpHierRNNCell(
            ...
            prior_memory_slots=prior_memory_slots,
            posterior_memory_slots=posterior_memory_slots,
            prior_core=prior_core,
        ...
        super(TmpHierRNN, self).__init__(
            cell,
    ...
    def call(self, inputs, mask=None, training=None, initial_state=None):
        ...
        return super(TmpHierRNN, self).call(

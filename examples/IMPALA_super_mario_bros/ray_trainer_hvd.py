...
flags.DEFINE_string(
    "basedir_hdfs",
    'hdfs://haruna/home/byte_arnold_lq_mlsys'
    '/user/xiaochangnan/test_ppo', "base dir for hdfs")
flags.DEFINE_string("dir", "vtrace0", "dir number")
flags.DEFINE_string(
    "scriptdir",
    "/opt/tiger/test_ppo/examples/IMPALA_super_mario_bros",
    "script dir")
...
from examples.IMPALA_super_mario_bros.policy_graph import warp_Model
...
def build_learner(pre, post, act_space, num_frames):
    ...
    pre_model = Model(
        act_space, gamma, rnn, use_rmc, use_hrmc or use_hrnn,
...
def build_policy_evaluator(kwargs):
    ...
    # pickle func in func
    from examples.IMPALA_super_mario_bros.env import build_env
...
def train():
    ...
    ps = AsyncPS.remote()
    small_data_collector = RolloutCollector(server_nums=1, ps=ps, policy_evaluator_build_func=build_policy_evaluator,
                                            **kwargs)
    cache_struct_path = '/tmp/%s_hvd.pkl' % FLAGS.dir

...
def build_learner(pre, post, act_space, num_frames):
    ...
    pre_model = Model(
        act_space, gamma, rnn, use_hrnn,
        use_reward_prediction, after_rnn, use_pixel_control,
        use_pixel_reconstruction, False, **pre)
    ...
    post_model = Model(
        act_space, gamma, rnn, use_hrnn,
        use_reward_prediction, after_rnn, use_pixel_control,
        use_pixel_reconstruction, True, **post)
    ...
    if use_reward_prediction:
        r_loss = tf.reduce_mean(
            mse(post_model.reward_prediction, post_model.r) * post_model.mask[:, :-1])
    ...
    if use_pixel_reconstruction:
        ...
        rec_loss = tf.reduce_mean(
            mse(post_model.pixel_reconstruction,
                s[:, :-1, :, :, :]
            ) * post_model.mask[:, :-1, None, None, None])
    ...
    for k, v in dequeued.items():
        if k == "state_in":
            ...
            prephs[k], postphs[k] = tf.split(
                v, [FLAGS.burn_in, FLAGS.seqlen + 1], axis=1)

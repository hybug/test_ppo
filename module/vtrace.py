...
VTraceFromLogitsReturns = namedtuple(
    ...
    ['vs', 'advantages', 'pg_advantages', 'log_rhos',
     'behaviour_action_log_probs', 'target_action_log_probs'])

VTraceReturns = namedtuple('VTraceReturns', ['vs', 'advantages', 'pg_advantages'])
...
def from_logits(
    ...
    target_policy_logits = tf.transpose(target_policy_logits, perm=[1, 0, 2])
    ...
    with tf.name_scope(name, values=[
        ...
        behaviour_action_log_probs = tf.transpose(behaviour_action_log_probs, perm=[1, 0])
        target_action_log_probs = tf.transpose(target_action_log_probs, perm=[1, 0])
        ###########################################
...
def from_importance_weights(
    ...
    with tf.name_scope(name, values=[
        ...
        cs = tf.clip_by_value(rhos, 0.95, 1.0, name="cs")
        ...
        pg_advantages = clipped_pg_rhos * advantages

        ###########################################
        vs = tf.transpose(vs, perm=[1, 0])
        advantages = tf.transpose(advantages, perm=[1, 0])
        pg_advantages = tf.transpose(pg_advantages, perm=[1, 0])
        ###########################################

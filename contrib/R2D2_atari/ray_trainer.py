...
from contrib.R2D2_atari.policy_graph import warp_Model
from contrib.R2D2_atari.game_list import get_games
...
flags.DEFINE_string(
    "basedir_ceph", "/mnt/cephfs_new_wj/arnold/labcv/xiaochangnan"
                    "/PPOcGAE_atari", "base dir for ceph")
flags.DEFINE_string(
    "basedir_hdfs",
    'hdfs://haruna/home/byte_arnold_lq_mlsys/user/xiaochangnan/atari',
    "base dir for hdfs")
flags.DEFINE_string("dir", "R2D2_0", "dir number")
flags.DEFINE_string(
    "scriptdir",
    "/opt/tiger/test_ppo/contrib/R2D2_atari",
    "script dir")

flags.DEFINE_bool("use_stage", False, "whether to use tf.contrib.staging")

flags.DEFINE_integer("use_soft", 0, "whether to use soft")
...
flags.DEFINE_float("qf_coef", 100.0, "weight of value fn loss")
...
def build_learner(pre, post, ws, act_space, num_frames):
    ...
    pre_model = Model(
        act_space, gamma, n_step, rnn, use_hrnn, use_rmc, use_amc, use_reward_prediction,
    ...
    if use_soft:
        ...
        target_entropy = tf.train.polynomial_decay(
            ...
            FLAGS.total_environment_frames // (
                    FLAGS.batch_size * FLAGS.seqlen),
            0.5 * np.log(act_space), power=1.5)
...
def build_policy_evaluator(kwargs):
    ...
    # pickle func in func
    from contrib.R2D2_atari.env import build_env
    from contrib.R2D2_atari.policy_graph import build_evaluator_model
...
def train():
    ...
    while total_frames < FLAGS.total_environment_frames:
        ...
        datas, is_weights, idxes = replay_buffer.sample(FLAGS.batch_size, sample_beta)

        fd = {phs[k]: datas[k] for k in keys}
        fd[weights] = is_weights
        ...
        replay_buffer.update_priorities(idxes, p)

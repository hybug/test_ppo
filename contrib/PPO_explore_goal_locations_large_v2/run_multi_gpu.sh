...
SCRIPT_DIR='/opt/tiger/test_ppo/contrib/PPO_explore_goal_locations_large_v2'
...
if [[ $role != "server" ]]; then
  ...
  ray start --head --include-webui --node-ip-address=$SERVER_IP --redis-port=${REDIS_PORT} --num-cpus=$((ARNOLD_SERVER_CPU - cpu_remain_for_head)) --memory=$(($ARNOLD_SERVER_MEM * 1024 ** 2)) --object-store-memory=$((ARNOLD_SERVER_GPU * 60 * 1024 ** 3))

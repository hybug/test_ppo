...
class DemoEnv(object):
    ...
    def get_history(self):
        if self.done:
            traj = {"s": self.s, "a": self.a, "r": self.r}
            return traj
        return None
...
def run():
    CKPT_DIR = "/".join(os.getcwd().split("/")[:-2]) + "/ckpt/casa12_800"
    # CKPT_DIR = "casa12_800"
...
def make_demo():
    CKPT_DIR = "/".join(os.getcwd().split("/")[:-2]) + "/ckpt/casa0"
    ...
    game = "SuperMarioBros-2-4-v0"
    env = DemoEnv(game, **kwargs)
    env.reset()

    def step(a=None, env=env):
        if a is None:
            fd = dict()
            fd[model.s] = [[env.get_state()]]
            fd[model.a] = [[env.get_act()]]
            fd[model.r] = [[env.get_reward()]]
            fd[model.state_in] = [env.get_state_in()]
            ...
            t = env.step(a[0][0], state_in[0])
        else:
            t = env.step(a, np.zeros(kwargs["state_size"]))

        if t is not None:
            with open(os.path.join(CKPT_DIR, "Demo_%s.pkl" % game), "wb") as f:
                pickle.dump(t, f)

...
flags.DEFINE_string(
    ...
    "base dir for hdfs")
flags.DEFINE_string("dir", "CASA_0", "dir number")
flags.DEFINE_string(
    "scriptdir",
    "/opt/tiger/test_ppo/examples/CASA_super_mario_bros",
    "script dir")

flags.DEFINE_bool("use_stage", False, "whether to use tf.contrib.staging")

flags.DEFINE_integer("use_demo", 0, "whether to use demo")
...
flags.DEFINE_integer("use_epsilon_greedy", 0, "whether to use epsilon greedy")
flags.DEFINE_integer("use_beta", 0, "whether to use beta")
...
flags.DEFINE_integer("use_retrace", 0, "whether to use retrace to estimate q")
flags.DEFINE_integer("smooth_update", 0, "whether to smooth update q target")
...
from examples.CASA_super_mario_bros.policy_graph import warp_Model
from examples.CASA_super_mario_bros.env import padding
...
def build_demo_buffer(keys, alpha):
    ...
    def postprocess_one_seg(seg):
        ...
        next_seg["a_logits"] = np.zeros((seqlen, int(FLAGS.act_space)), np.float32)
        next_seg["v_cur"] = np.zeros(seqlen, np.float32)
...
def build_policy_evaluator(kwargs):
    ...
    # pickle func in func
    from examples.CASA_super_mario_bros.env import build_env
    from examples.CASA_super_mario_bros.policy_graph import build_evaluator_model
...
def train():
    ...
    while total_frames < FLAGS.total_environment_frames:
        ...
        if FLAGS.use_demo:
            ...
            fd[batch_weights] = np.concatenate([np.ones(batch_size),
            ...
            total_frames, gs, summary, _, p, sample_beta, dequeued_datas = sess.run(
                [num_frames_and_train, global_step_and_train,
                 summary_ops, stage_op, priority, beta, dequeued],
                feed_dict=fd)

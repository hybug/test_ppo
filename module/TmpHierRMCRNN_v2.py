...
class TmpHierRMCRNNCell_v2(DropoutRNNCellMixin, Layer):
    def __init__(self,
                 time_scale,
                 size_per_head,
                 num_attention_heads,
                 ...
                 recurrent_dropout=0.,
                 **kwargs):
        super(TmpHierRMCRNNCell_v2, self).__init__(**kwargs)
    ...
    def build(self, input_shape):
        ...
        if self.use_relative_position:
            self.p_rel_table = self._make_relative_position_embedding_table(
                self.prior_memory_slots)
            self.p_pos_table = self._make_relative_position_table(
                self.prior_memory_slots)
            self.q_rel_table = self._make_relative_position_embedding_table(
                self.posterior_memory_slots)
            self.q_pos_table = self._make_relative_position_table(
                self.posterior_memory_slots)
        self.built = True
    ...
    def _attention_layer(self, memory_plus_inputs, ws, num_memory_slots, rel_table, pos_table):
        ...
        if self.use_relative_position:
            # [F+T-1, N*H]
            r = K.dot(rel_table, ws["rel_kernel"])
            # [F+T-1, N, H]
            r = array_ops.reshape(
            ...
            # [B*N, F, T]
            bd = tf.einsum("bfl,ftl->bft", bd, pos_table)
            # [B*N, F, T]
            attention_scores += bd
    ...
    def _attend_over_memory(self, inputs, memory, ws, num_memory_slots, rel_table, pos_table):
        ...
        context_layer = self._attention_layer(
            memory_plus_inputs, ws, num_memory_slots, rel_table, pos_table)
    ...
    def _call_one_layer(self, inputs, flatten_memory, training, ws, num_memory_slots, rel_table, pos_table):
        ...
        hs, new_memory = self._attend_over_memory(
            inputs, memory, ws, num_memory_slots, rel_table, pos_table)
    ...
    def call(self, inputs, states, training=None):
        ...
        (p_next_zs, p_next_mus, p_next_sigmas,
         p_next_hs, p_next_flatten_memory) = self._call_one_layer(
            ...
            self.p_rel_table, self.p_pos_table)
        ...
        (q_next_zs, q_next_mus, q_next_sigmas,
         q_next_hs, q_next_flatten_memory) = self._call_one_layer(
            ...
            self.q_rel_table, self.q_pos_table)
    ...
    def get_config(self):
        ...
        base_config = super(TmpHierRMCRNNCell_v2, self).get_config()
...
class TmpHierRMCRNN_v2(RNN):
    def __init__(self,
        ...
        super(TmpHierRMCRNN_v2, self).__init__(
            cell,
    ...
    def call(self, inputs, mask=None, training=None, initial_state=None):
        ...
        return super(TmpHierRMCRNN_v2, self).call(
    ...
    def get_config(self):
        ...
        base_config = super(TmpHierRMCRNN_v2, self).get_config()

...
if [[ $role != "server" ]]; then
  ...
  ray start --head --include-webui --node-ip-address=$SERVER_IP --redis-port=${REDIS_PORT} --num-cpus=$((ARNOLD_SERVER_CPU - cpu_remain_for_head)) --memory=$(($ARNOLD_SERVER_MEM * 1024 ** 2)) --object-store-memory=$((ARNOLD_SERVER_GPU * 16 * 1024 ** 3)) --plasma-directory /dev/shm
  ...
  sleep 10s
  python3 $SCRIPT_DIR/ray_trainer.py "$@"
